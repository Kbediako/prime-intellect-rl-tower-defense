...
## Project-specific run hygiene (Prime TD)
- Record run IDs + config filenames immediately after `prime rl run` (before any analysis).
- Pull rollouts for **every step with samples** and parse **all rollouts** at each step (use the highest available limit, e.g. 32).
- Parse **every** user/assistant turn in rollouts; never rely on first/last only.
